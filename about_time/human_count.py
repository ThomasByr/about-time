...
def __human_count(val: float, unit: str, prec: int | None, space: str, divisor: int, spec: tuple) -> str:
    for scale, dec in zip(spec, DECIMALS):  # noqa: B007, B905
        r = round(val, dec)
        if r >= divisor:
            val /= divisor
            continue
    ...
    if prec is not None:
        ...
        prec = 2
    return f"{r:.{prec}f}{space}{scale}{unit}"


def fn_human_count(space: bool, d1024: bool, iec: bool):
    def run(val: float, unit: str, prec: int | None):
...
class HumanCount:
    ...
    def as_human(self, prec: int | None = None) -> str:
